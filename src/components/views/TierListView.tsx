  ...
  return (
    <div className="flex flex-col w-full max-w-6xl mx-auto pb-20 border-b border-border">
        ...
        >
          ...
          >
            {editingLabelId === row.id ? (
              <textarea
                ...
                className="w-full h-full bg-transparent text-center text-gray-700 font-black text-xl resize-none outline-none overflow-hidden"
              />
            ) : (
              <span className="text-center text-gray-700 font-black text-xl break-words leading-tight w-full select-none">
                {row.label}
              </span>
            )}
          </div>
          <TierRowWrapper rowId={row.id} justify={rank.gridJustify}>
            <AnimatePresence>
          ...
          <div className="w-10 sm:w-16 bg-surface shrink-0 flex flex-col items-center justify-between py-1 border-l border-border">
            <Select
              value=""
              onChange={(val) => {
                if (val === "clear") handleUpdateRow(row.id, { items: [] });
      ...
      >
        <Plus
          size={24}
          className="group-hover/add-row:scale-110 transition-transform"
        />
      </button>
    </div>
  );
