...
interface TierListViewProps {
  ...
  onInboxDrop: (itemId: string, collectionId: string, rowId: string, itemIndex: number) => void;
...
const TierItem: React.FC<{
    ...
    onInteract: (rowId: string, itemId: string) => void;
    onMoveToInbox: (rowId: string, itemIndex: number) => void;
    handleItemDrop: (e: React.DragEvent, targetRowId: string, targetIndex: number) => void;
    onUpdateItem: (rowId: string, itemId: string, updates: Partial<CellData>) => void;
    onInboxDrop: (itemId: string, collectionId: string, rowId: string, itemIndex: number) => void;
    onInboxDropMulti: (itemIds: string[], collectionId: string, rowId: string, itemIndex: number) => void;
    onInternalMove: (sourceRowId: string, sourceItemId: string, targetRowId: string, targetIndex: number) => void;
    onSearchDrop: (imageSrc: string, rowId: string, itemIndex: number) => void;
    ...
    const [posY, setPosY] = useState(item.objectPosition ? parseInt(item.objectPosition.split(' ')[1]) : 50);
    ...
    return (
        <div
            ...
            onClick={(e) => {
                ...
                if (interactionState?.type === 'inbox') {
                    ...
                    onInboxDropMulti(interactionState.itemIds, interactionState.collectionId, rowId, idx);
                } else if (interactionState?.type === 'tier-item') {
                    onInternalMove(interactionState.rowId, interactionState.itemId, rowId, idx);
            ...
            onDragStart={(e) => {
                e.dataTransfer.setData('application/json', JSON.stringify({ type: 'tier-item', rowId: rowId, itemId: item.id }));
        ...
        >
            ...
            `}>
                <img src={getProxiedImageUrl(item.imageSrc!)} className="w-full h-full object-cover pointer-events-none transition-all duration-300" style={objectPosStyle} referrerPolicy="no-referrer" />
                ...
                {!isAdjusting && (
                    <div className="absolute top-0 right-0 hidden md:flex gap-1 p-0.5 opacity-0 group-hover/item:opacity-100 transition-opacity export-hidden">
                        ...
                        <button
                            ...
                            onClick={(e) => { e.stopPropagation(); onMoveToInbox(rowId, idx); }}
                            title="Remove"
                        >
                            <X size={12} />
                        </button>
                ...
                {isAdjusting && (
                    <div className="export-hidden adjust-controls absolute inset-0 bg-black/60 backdrop-blur-sm flex flex-col items-center justify-center p-1 gap-2 z-30">
                        <div className="w-full flex flex-col gap-1 bg-black/40 p-1 rounded-md border border-white/10">
                            <input type="range" min="1" max="3" step="0.1" value={zoom} onChange={(e) => setZoom(parseFloat(e.target.value))} className="w-full h-1 accent-primary" title="Zoom" />
                            <input type="range" min="0" max="100" value={posX} onChange={(e) => setPosX(parseInt(e.target.value))} className="w-full h-1 accent-primary" title="Pan X" />
                            <input type="range" min="0" max="100" value={posY} onChange={(e) => setPosY(parseInt(e.target.value))} className="w-full h-1 accent-primary" title="Pan Y" />
                        </div>
                        <div className="flex gap-1">
                            <button onClick={(e) => { e.stopPropagation(); setIsAdjusting(false); }} className="p-1 bg-white/10 hover:bg-white/20 text-white rounded-sm transition-colors">
                                <X size={12} />
                            </button>
                            <button onClick={saveAdjustments} className="p-1 bg-primary hover:bg-primary/80 text-white rounded-sm transition-colors">
                                <Check size={12} />
                            </button>
                        </div>
                    </div>
...
export const TierListView: React.FC<TierListViewProps> = ({
  ...
  const handleClearRow = (rowId: string) => {
      const newRows = rank.tierRows.map(r => r.id === rowId ? { ...r, items: [] } : r);
      onUpdateTierRows(newRows);
  };
  ...
  const handleRowClick = (rowId: string) => {
      ...
      if (interactionState.type === 'inbox') {
          onInboxDrop(interactionState.itemId, interactionState.collectionId, rowId, -1);
      } else if (interactionState.type === 'inbox-multi') {
          onInboxDropMulti(interactionState.itemIds, interactionState.collectionId, rowId, -1);
      } else if (interactionState.type === 'tier-item') {
          onInternalMove(interactionState.rowId, interactionState.itemId, rowId, -1);
      }
  };
  ...
  const handleItemDrop = (e: React.DragEvent, targetRowId: string, targetIndex: number) => {
      ...
      try {
          ...
          if (data.type === 'inbox') {
              ...
              onInboxDropMulti(data.ids, data.originCollectionId, targetRowId, targetIndex);
          } else if (data.type === 'search') {
    ...
    <div className="flex flex-col w-full max-w-6xl mx-auto pb-20 border-b border-border">
      {rank.tierRows.map((row, rowIndex) => (
        <div key={row.id} className="flex min-h-[6rem] border-t border-border bg-surface">
          ...
          {/* 3. Controls (Right) */}
          <div className="w-10 sm:w-12 bg-surface shrink-0 flex flex-col items-center justify-between py-1 border-l border-border export-hidden">
             <div className="relative">
                 <button
                    onClick={() => setSettingsOpenId(settingsOpenId === row.id ? null : row.id)}
                 ...
                 >
                  ...
                  {/* Settings Popover */}
                 {settingsOpenId === row.id && (
                    <div className="absolute top-0 right-full mr-2 z-50 bg-[#18181b] border border-white/10 shadow-2xl rounded-xl p-4 w-56 flex flex-col gap-3 animate-in fade-in zoom-in-95 origin-top-right">
                        <div className="flex justify-between items-center mb-1">
                            <span className="text-xs font-bold text-muted uppercase tracking-wider">Row Settings</span>
                            <button onClick={() => setSettingsOpenId(null)} className="text-muted hover:text-white"><X size={14}/></button>
                        </div>

                        {/* Actions */}
                        <div className="flex flex-col gap-1">
                            <button onClick={() => handleClearRow(row.id)} className="flex items-center gap-2 text-xs p-2 hover:bg-white/5 rounded-lg text-left text-orange-400 font-bold transition-colors">
                                <Eraser size={14} /> Clear Images
                            </button>
                            <button onClick={() => handleDeleteRow(rowIndex)} className="flex items-center gap-2 text-xs p-2 hover:bg-red-500/10 rounded-lg text-left text-red-400 font-bold transition-colors">
      ...
      {/* Add Row Button */}
      <div className="bg-surface p-6 flex justify-center export-hidden">
         <button
            onClick={handleAddRow}
            className="group flex items-center justify-center gap-2 px-8 py-4 bg-surface border border-border rounded-2xl text-muted hover:text-text hover:border-primary/50 hover:bg-white/5 transition-all font-bold text-sm shadow-sm hover:shadow-lg hover:shadow-primary/5 hover:-translate-y-0.5"
         >
            <div className="p-1 bg-primary/10 rounded-full text-primary group-hover:bg-primary group-hover:text-white transition-colors">
