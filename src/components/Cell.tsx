...
import { Plus, X, Upload, Download, ArrowUp, ArrowDown, Minus, Move, Crop, Check } from 'lucide-react';
...
export const Cell: React.FC<CellProps> = ({ 
  ...
  return (
    <div
      className={`
        relative group/cell overflow-hidden transition-all duration-200 aspect-[3/4]
        ${roundedClass}
        ${bgClass}
        ${outlineClass}
        ${isDragOver ? 'ring-4 ring-primary bg-primary/20 z-20 scale-105 shadow-2xl' : ''}
        ${isSelected ? 'ring-2 ring-primary z-10 shadow-xl scale-[0.98]' : ''}
        ${!data.imageSrc ? 'cursor-pointer hover:bg-hover' : 'cursor-pointer'}
    ...
    >
      ...
      {data.imageSrc ? (
        <>
          ...
          {/* Overlay Controls */}
          {!isAdjusting && (
            <div className={`export-hidden absolute inset-0 bg-black/40 opacity-0 group-hover/cell:opacity-100 ${isSelected ? 'opacity-0' : ''} transition-all duration-200 flex flex-col items-center justify-center gap-2 backdrop-blur-[2px]`}>
          ...
          {/* Adjust Controls */}
          {isAdjusting && (
            <div className="export-hidden adjust-controls absolute inset-0 bg-black/60 backdrop-blur-sm flex flex-col items-center justify-center p-4 gap-4 z-30">
        ...
        <div className="w-full h-full flex flex-col items-center justify-center text-muted group-hover/cell:text-text transition-colors export-hidden pointer-events-none">
