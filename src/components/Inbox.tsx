        ...
        >
            ...
            <div className="absolute inset-0 bg-black/40 flex items-center justify-center pointer-events-none backdrop-blur-[2px]">
                <div className="bg-white/20 rounded-full p-2 backdrop-blur-md">
                    <Check size={24} className="text-white drop-shadow-md" strokeWidth={2.5} />
                </div>
            </div>
            )}
...
export const Inbox: React.FC<InboxProps> = ({
  ...
  const currentItems = activeCollectionId === 'all-images' ? allItems : (activeCollection?.items || []);
  ...
  const handleDeleteItem = (item: InboxItem) => {
      ...
      if (isAllView) {
          ...
          const foundCol = collections.find(c => c.items.some(i => i.id === item.id));
                ...
                {activeTab === 'picker' && (
                    <div className="absolute inset-0 bg-[#1c1c1e] flex flex-col p-6 animate-in fade-in slide-in-from-right-4 z-20">
                        <div className="flex items-center gap-3 mb-6 shrink-0">
                            <button onClick={() => setActiveTab('search')} className="p-2 hover:bg-white/10 rounded-full text-blue-500 hover:text-blue-400 transition-colors">
                                <ArrowLeft size={20} />
                            </button>
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 overflow-y-auto custom-scrollbar">
                            {collections.map(col => (
                                ...
                                >
                                    ...
                                    <span className="font-medium text-[13px] line-clamp-2">{col.name}</span>
                                    {col.id === lastTargetCollectionId && <span className="text-[11px] font-medium text-blue-500/70">Recent</span>}
                                </button>
                            ))}
                            <button
                              onClick={() => { onAddCollection(); }}
                              className="flex flex-col items-center justify-center p-4 rounded-2xl border border-dashed border-white/20 hover:border-blue-500 hover:text-blue-500 transition-all gap-3 text-center h-32 text-white/50 hover:bg-blue-500/5"
                            >
                                    ...
                                    >
                                      {isEditing ? (
                                        <input
                                          ...
                                          onBlur={() => {
                                            if(tempName.trim()) onRenameCollection(col.id, tempName);
