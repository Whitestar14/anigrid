...
export interface AppState extends GlobalState {
  ...
  handleVisualToggle: (key: 'showNumbers' | 'showTitle' | 'showDate' | 'showTiers' | 'borderless') => void;
  ...
  handleInboxDropToTier: (itemId: string, collectionId: string, rowId: string, targetIndex: number) => void;
  ...
  handleUpdateLastTarget: (collectionId: string) => void;
...
const checkAndRescueImages = (removedCells: (CellData | InboxItem)[], currentState: GlobalState, _collectionToAddToId: string) => {
  ...
  removedCells.forEach((cell, idx) => {
    if (cell.imageSrc && !allKnownImages.has(cell.imageSrc)) {
      rescueItems.push({ id: `rescued-${Date.now()}-${idx}`, imageSrc: cell.imageSrc, createdAt: Date.now() });
    }
  });
  return rescueItems;
};
...
const defaultState: GlobalState = {
  ...
  ranks: {
    'default': {
      ...
      cells: Array.from({ length: 9 }).map((_, i) => ({ id: `cell-${i}`, imageSrc: null, position: i })),
      style: 'seamless',
      showNumbers: true,
      showTitle: true,
      showDate: true,
...
export const useStore = create<AppState>()(
  temporal(
    persist(
      ...
      duplicateModalConfig: { isOpen: false, imageSrc: null, actionToExecute: null },

      setInteractionState: (state) => set({ interactionState: state }),
      setDuplicateModalConfig: (config) => set((state) => ({ duplicateModalConfig: { ...state.duplicateModalConfig, ...config } })),
      ...
      checkDuplicateAndProceed: (imageSrc, action) => {
        ...
        if (activeRank.type === 'ranking') {
          ...
          isDuplicate = activeRank.tierRows.some(row => row.items.some(item => item.imageSrc === imageSrc));
        }

        if (isDuplicate) {
          set({ duplicateModalConfig: { isOpen: true, imageSrc, actionToExecute: action } });
        } else {
          action();
        }
      },
      ...
      handleDuplicateConfirm: (dontAskAgain) => {
        ...
        if (dontAskAgain) {
          set((s) => ({ preferences: { ...s.preferences, skipDuplicateWarning: true } }));
        }
      ...
      updateActiveRank: (updates) => set((state) => {
        ...
        return {
          ranks: {
            ...state.ranks,
            [state.activeRankId]: { ...activeRank, ...updates, updatedAt: Date.now() }
          }
        };
      }),

      updateRankById: (id, updates) => set((state) => {
      ...
      updateActiveCollection: (updates) => set((state) => {
        return {
          inbox: {
            ...state.inbox,
      ...
      handleConfigChange: (newConfig) => set((state) => {
        ...
        if (newTotal > oldTotal) {
          for (let i = oldTotal; i < newTotal; i++) {
            nextCells.push({ id: `cell-${Date.now()}-${i}`, imageSrc: null, position: i });
          }
          return {
            ranks: {
              ...state.ranks,
              [state.activeRankId]: { ...activeRank, config: newConfig, cells: nextCells, updatedAt: Date.now() }
            }
          };
        ...
        return {
          ranks: {
            ...state.ranks,
            [state.activeRankId]: { ...activeRank, config: newConfig, updatedAt: Date.now() }
          }
        };
      }),
      ...
      handleModeChange: (newMode) => set((state) => {
        ...
        return {
          ranks: {
            ...state.ranks,
            [state.activeRankId]: { ...activeRank, mode: newMode, config: newConfig, updatedAt: Date.now() }
          }
        };
      }),

      handleVisualToggle: (key) => set((state) => {
        const activeRank = state.ranks[state.activeRankId];
        if (!activeRank) return state;
        return {
          ranks: {
            ...state.ranks,
            [state.activeRankId]: { ...activeRank, [key]: !activeRank[key], updatedAt: Date.now() }
          }
        };
      }),

      handleCellUpload: (index, dataUrl) => set((state) => {
        const activeRank = state.ranks[state.activeRankId];
        if (!activeRank) return state;
        const newCells = activeRank.cells.map((cell, i) => i === index ? { ...cell, imageSrc: dataUrl } : cell);
        return {
          ranks: {
            ...state.ranks,
            [state.activeRankId]: { ...activeRank, cells: newCells, updatedAt: Date.now() }
          }
        };
      }),

      handleUpdateCell: (index, data) => set((state) => {
        const activeRank = state.ranks[state.activeRankId];
        if (!activeRank) return state;
        const newCells = activeRank.cells.map((cell, i) => i === index ? { ...cell, ...data } : cell);
        return {
          ranks: {
            ...state.ranks,
            [state.activeRankId]: { ...activeRank, cells: newCells, updatedAt: Date.now() }
      ...
      handleCellClear: (index) => set((state) => {
        const activeRank = state.ranks[state.activeRankId];
        if (!activeRank) return state;
      ...
      handleSwapCells: (fromIndex, toIndex) => set((state) => {
        const activeRank = state.ranks[state.activeRankId];
        if (!activeRank) return state;
        const newCells = [...activeRank.cells];
        const temp = { ...newCells[fromIndex] };
        newCells[fromIndex] = { ...newCells[toIndex], id: newCells[fromIndex].id, position: fromIndex };
        newCells[toIndex] = { ...temp, id: newCells[toIndex].id, position: toIndex };
        return {
          ranks: {
            ...state.ranks,
            [state.activeRankId]: { ...activeRank, cells: newCells, updatedAt: Date.now() }
      ...
      handleMoveToInbox: (index) => set((state) => {
        const activeRank = state.ranks[state.activeRankId];
        if (!activeRank) return state;
        const cell = activeRank.cells[index];
        const targetColId = state.inbox.activeCollectionId === 'all-images' ? state.inbox.collections[0].id : state.inbox.activeCollectionId;
      ...
      handleUpdateTierRows: (rows) => set((state) => {
        const activeRank = state.ranks[state.activeRankId];
        if (!activeRank) return state;
        return {
          ranks: {
            ...state.ranks,
            [state.activeRankId]: { ...activeRank, tierRows: rows, updatedAt: Date.now() }
          }
        };
      }),

      handleTierItemRemove: (rowId, itemId) => set((state) => {
        const activeRank = state.ranks[state.activeRankId];
        if (!activeRank) return state;
        ...
        return {
          ...
          ranks: {
            ...state.ranks,
            [state.activeRankId]: { ...activeRank, tierRows: newRows, updatedAt: Date.now() }
          }
        };
      }),

      handleTierMoveToInbox: (rowId, itemIndex) => set((state) => {
        const activeRank = state.ranks[state.activeRankId];
        if (!activeRank) return state;
        const row = activeRank.tierRows.find(r => r.id === rowId);
        if (!row) return state;
      ...
      handleInboxDropToTier: (itemId, collectionId, rowId, targetIndex) => {
        ...
        get().checkDuplicateAndProceed(item.imageSrc, () => {
          set((s) => {
            ...
            const newCellData: CellData = { id: `tier-cell-${Date.now()}`, imageSrc: item!.imageSrc, position: 0 };
      ...
      handleSearchDropToTier: (imageSrc, rowId, targetIndex) => {
        get().checkDuplicateAndProceed(imageSrc, () => {
          set((s) => {
            const currentRank = s.ranks[s.activeRankId];
            if (!currentRank) return s;
            const newCellData: CellData = { id: `tier-cell-${Date.now()}`, imageSrc: imageSrc, position: 0 };
            const newRows = currentRank.tierRows.map(r => {
                if (r.id === rowId) {
                    const newItems = [...r.items];
                    const insertIdx = targetIndex === -1 ? newItems.length : targetIndex;
            ...
            return {
              ...
              ranks: { ...s.ranks, [s.activeRankId]: { ...currentRank, tierRows: newRows, updatedAt: Date.now() } }
            };
          });
        });
      },
      ...
      handleInternalTierMove: (sourceRowId, sourceItemId, targetRowId, targetIndex) => set((state) => {
        ...
        let newRows = activeRank.tierRows.map(r => {
            if (r.id === sourceRowId) {
                ...
                return { ...r, items: r.items.filter(i => i.id !== sourceItemId) };
            }
            return r;
        });
      ...
      handleInboxDropMulti: (itemIds, sourceColId, toIndex) => {
        ...
        set((s) => {
          ...
          for (const item of itemsToDrop) {
              ...
              while (currentDropIndex < newCells.length && newCells[currentDropIndex].imageSrc) {
                  currentDropIndex++;
              }
              if (currentDropIndex < newCells.length) {
                  newCells[currentDropIndex] = { ...newCells[currentDropIndex], imageSrc: item.imageSrc };
      ...
      handleInboxDropToTierMulti: (itemIds, collectionId, rowId, targetIndex) => {
        ...
        set((s) => {
          ...
          for (const item of itemsToDrop) {
              if (!s.preferences.skipDuplicateWarning) {
                  const isDuplicate = currentRank.tierRows.some(row => row.items.some(i => i.imageSrc === item.imageSrc));
        ...
        get().checkDuplicateAndProceed(imageSrc, () => {
          set((s) => {
            ...
            return {
              ...
              ranks: { ...s.ranks, [s.activeRankId]: { ...currentRank, cells: newCells, updatedAt: Date.now() } }
            };
          });
        });
      },

      handleAddToCollection: (imageSrc, collectionId) => set((state) => {
        const newItem: InboxItem = { id: `inbox-add-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`, imageSrc, createdAt: Date.now() };
        return {
          inbox: {
            ...state.inbox,
      ...
      handleUpdateLastTarget: (collectionId) => set((state) => ({
        inbox: { ...state.inbox, lastTargetCollectionId: collectionId }
      })),

      handleRestoreItem: (item, collectionId) => set((state) => {
        const targetId = state.inbox.collections.find(c => c.id === collectionId) ? collectionId : state.inbox.collections[0].id;
        return {
          inbox: {
            ...state.inbox,
            collections: state.inbox.collections.map(c =>
      ...
      switchCollection: (id) => set((state) => ({
        inbox: { ...state.inbox, activeCollectionId: id }
      })),

      addCollection: () => set((state) => {
        const newId = `col-${Date.now()}`;
        return {
          inbox: {
            ...state.inbox,
            activeCollectionId: newId,
            collections: [...state.inbox.collections, { id: newId, name: 'New Collection', items: [] }]
          }
        };
      }),
      ...
      deleteCollection: (id) => set((state) => {
        ...
        return {
          inbox: {
            ...state.inbox,
            collections: newCols,
            activeCollectionId: state.inbox.activeCollectionId === id ? newCols[0].id : state.inbox.activeCollectionId
          }
        };
      }),
      ...
      handleClearAll: () => set((state) => {
        ...
        if (activeRank.type === 'tierlist') {
            const allItems: CellData[] = activeRank.tierRows.flatMap(r => r.items);
            ...
            const newRows = activeRank.tierRows.map(r => ({ ...r, items: [] }));
            return {
              inbox: newInbox,
              ranks: {
                ...state.ranks,
            ...
            const newCells = activeRank.cells.map(c => ({ ...c, imageSrc: null, textLabel: undefined, rating: undefined }));
            return {
              inbox: newInbox,
              ranks: {
                ...state.ranks,
